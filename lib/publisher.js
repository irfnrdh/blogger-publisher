...
async function processLocalImages(markdownContent, baseDir, authClient) {
  ...
  for (const match of matches) {
    ...
    // Cek apakah imagePath adalah link lokal dan bukan Data URI
    if (!imagePath.startsWith('http://') && !imagePath.startsWith('https://') && !imagePath.startsWith('data:')) {
      try {
        const decodedPath = decodeURIComponent(imagePath);
        const absoluteImagePath = path.resolve(baseDir, decodedPath);
        
        // SECURITY: Cegah Path Traversal (gambar harus berada di dalam Current Working Directory)
        const cwd = process.cwd();
        if (!absoluteImagePath.startsWith(cwd)) {
            console.warn(`    ⚠️ [SECURITY] Melewati gambar ${imagePath} karena berada di luar direktori kerja (Path Traversal Risk).`);
            result += fullMatch;
            continue;
        }
        ...
        console.warn(`    ⚠️ Melewati gambar ${imagePath} karena error upload: ${error.message}`);
      }
    }
    result += fullMatch;
  }
