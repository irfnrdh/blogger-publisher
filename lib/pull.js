...
async function runPuller(targetDir, accountId, customBlogId) {
    if (!accountId) {
    ...
    const BLOG_ID = customBlogId || config.blogId;
    if (!BLOG_ID) {
        console.error("❌ ERROR: BLOG_ID tidak ditemukan (lewat CLI maupun config)");
        process.exit(1);
    }
