...
function createApiServer() {
  ...
  // CORS: localhost only — only set header if origin is local
...
function startApiServer(port = DEFAULT_PORT) {
  ...
  const server = app.listen(port, '127.0.0.1', () => {
    const line = (text) => `│ ${text.padEnd(49)} │`;
    console.log('');
    console.log('┌───────────────────────────────────────────────────┐');
    console.log('│    🚀  Blogger Publisher — Local API Server        │');
    console.log('├───────────────────────────────────────────────────┤');
    console.log(line(`  URL     : http://localhost:${port}`));
    console.log(line(`  Version : v${pkg.version}`));
    console.log('├───────────────────────────────────────────────────┤');
    console.log(line(`  🔑 Key  : ${apiKey.substring(0, 28)}...`));
    console.log(line(`  (full key: ~/.blogger-publisher/api.key)`));
    console.log('└───────────────────────────────────────────────────┘');
    console.log('');
  ...
  process.on('SIGINT', () => {
    ...
    // Force shutdown if connections (like SSE) keep it alive too long
    setTimeout(() => {
