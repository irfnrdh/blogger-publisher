...
Coba tambahkan gambar lokal Anda di folder \`images/\` dan panggil dengan \`![Gambar](../images/contoh.png)\`.`;
  ...
  if (!fs.existsSync(envPath)) {
    ...
    let envContent = `CLIENT_ID="${envAnswers.clientId}"\n`;
    envContent += `CLIENT_SECRET="${envAnswers.clientSecret}"\n`;
    envContent += `BLOG_ID="${envAnswers.blogId}"\n`;
    envContent += `IMAGE_PROVIDER="${envAnswers.imageProvider}"\n`;
    
    if (envAnswers.imageProvider === 'imgbb') {
      envContent += `\n#IMGBB_API_KEY="isi_api_key_disini"\n`;
      ...
      envContent += `\n#GITHUB_TOKEN="ghp_xxx"\n#GITHUB_REPO="username/repo-gambar"\n`;
    }

    fs.writeFileSync(envPath, envContent);
    p.note('File .env berhasil dibuat dengan konfigurasi Anda.', 'Setup Konfigurasi');
  } else {
    p.note('File .env sudah ada, di-skip.', 'Setup Konfigurasi');
  }
